...
def get_last_completed_watermark(engine: Engine) -> datetime | None:
    ...
    try:
        ...
        log.error(
            ...
            event="last_completed_watermark",
            error=str(exc),
        )
...
def ensure_checkpoint_run(
    ...
    try:
        ...
        log.info(
            "checkpoint_run_started",
            event="ensure_checkpoint_run",
...
def update_checkpoint_after_chunk(
    ...
    try:
        ...
        log.info(
            "checkpoint_updated",
            event="update_checkpoint_after_chunk",
            pipeline_run_id=pipeline_run_id,
...
def finalize_checkpoint_status(
    ...
    try:
        ...
        log.info(
            ...
            event="finalize_checkpoint_status",
            pipeline_run_id=pipeline_run_id,
            status=status,
        )
    except Exception as exc:
...
def _insert_dead_letter_inner(
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                INSERT INTO etl_failed_records (
...
def _clear_buffers_inner(engine: Engine, pipeline_run_id: str) -> None:
    with engine.begin() as conn:
        conn.execute(
            text("DELETE FROM etl_staging WHERE pipeline_run_id = :run_id"),
            {"run_id": pipeline_run_id},
        )
        conn.execute(
            text("DELETE FROM etl_validated WHERE pipeline_run_id = :run_id"),
            {"run_id": pipeline_run_id},
        )
