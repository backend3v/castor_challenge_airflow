...
from pipeline.exceptions import CheckpointError, ValidationError
from pipeline.log import configure_logging, get_logger
from pipeline.models.financial_record.model import FinancialRecord
from pipeline.resilience.checkpoint import insert_dead_letter
...
def _normalize_row_types(row: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, val in row.items():
        if key in {"transaction_id", "account_id"} and isinstance(val, str):
            out[key] = UUID(val)
        elif key == "amount":
