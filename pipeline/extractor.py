...
from pipeline.resilience.checkpoint import (
    clear_run_buffers,
...
from pipeline.serialize import records_json_dumps
...
def _insert_staging_chunk(
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                INSERT INTO etl_staging (pipeline_run_id, chunk_index, records)
...
def run_extraction(pipeline_run_id: str, settings: Settings | None = None) -> dict[str, Any]:
    ...
    try:
        extract_since, initial_wm = load_checkpoint_context(engine, cfg)
        clear_run_buffers(engine, pipeline_run_id)
        ensure_checkpoint_run(engine, pipeline_run_id, initial_wm)

        log.info(
            "extraccion_iniciada",
            event="run_extraction",
            pipeline_run_id=pipeline_run_id,
            extract_since=extract_since.isoformat(),
            chunk_size=cfg.chunk_size,
        )

        df = _rows_to_polars(engine, extract_since)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = [
                executor.submit(
                    _chunk_worker,
                    (engine, pipeline_run_id, idx, recs, semaphore),
                )
        ...
        set_total_chunks(engine, pipeline_run_id, total_chunks)
